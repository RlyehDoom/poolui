...
var app = angular.module('cryptominers-ui', [
	'pool.globals',
	'ngRoute',
	'ngMaterial',
	'md.data.table',
	...
	$routeProvider
		...
		.when('/help/support', {
			templateUrl: 'user/help/support.html',
			controller: 'SupportCtrl',
			activetab: 'support'
		})
		.when('/help/privacy', {
			templateUrl: 'user/help/privacy.html',
			controller: 'PrivacyCtrl',
			activetab: 'support'
		})
		.when('/help/service', {
			templateUrl: 'user/help/service.html',
...
app.controller('AppCtrl', function ($scope, $rootScope, $location, $route, $routeParams, $translate, $anchorScroll, $window, $interval, $mdDialog, dataService, timerService, addressService, $mdSidenav, $mdMedia, $localStorage, ngAudio, GLOBALS) {
	...
	$scope.closeSidenavIfMobile = function () {
		if ($mdMedia('gt-md')) {
			$mdSidenav('left').open();
			$scope.menuOpen = true;
		} else {
	...
	var loadData = function () {
		dataService.getData("/pool/stats", function (data) {
			...
			dataService.getData("/network/stats", function (data) {
				...
				if ($scope.poolStats.global != null && $scope.network != null) {

					var hashRateGlobal = Math.floor($scope.network.difficulty / 120);
					var hashRatePool = $scope.poolStats.global.hashRate;
					var lastReward = $scope.network.value / 1000000000000;
					var blocksPerDay = 740;

					var probToGetBlocksDaily = blocksPerDay * hashRatePool / hashRateGlobal;
					var averageDailyReward = probToGetBlocksDaily * lastReward;
					var daysToFindBlock = 1 / probToGetBlocksDaily;
					var probToGetBlocksWeekly = (probToGetBlocksDaily * 7) * 100;

					$scope.miningStatistics.hashRateGlobal = hashRateGlobal;
					$scope.miningStatistics.hashRatePool = hashRatePool;
					$scope.miningStatistics.lastReward = lastReward;
					$scope.miningStatistics.probToGetBlocksDaily = probToGetBlocksDaily;
					$scope.miningStatistics.averageDailyReward = averageDailyReward;
					$scope.miningStatistics.daysToFindBlock = daysToFindBlock;
					$scope.miningStatistics.probToGetBlocksWeekly = probToGetBlocksWeekly;
				}
			});
		});
